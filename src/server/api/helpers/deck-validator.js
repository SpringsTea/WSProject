...
module.exports = (deck) => {
  ...
  for (const cardID of deck.cards) {
    ...
    const card = deck.carddata.find( (c) => c._id == cardID );
    ...
    const cardNumber = `${card.set}/${card.side}${card.release}-${card.sid.replace(/[a-z]$/g, '')}`;
    // add card set to Set
  ...
  const setArray = Array.from(setCodes);
  const neoStandardSets = [];
  for (const neoStandardSet of Object.keys(neoStandardMap)) {
