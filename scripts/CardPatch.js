...
const setContent = JSON.parse(readFileSync(`${SET_PATH}/${SIDE}${RELEASE}.json`, {encoding: 'utf8'}));

// filter by sid if specified
const cardsToUpdate = setContent.filter( (cards) => {
  if (sid) {
...
cardsToUpdate.forEach( async (sourcecard) => {
  const remotecard = await CardModel.findOne({'side': sourcecard.side, 'release': sourcecard.release, 'sid': sourcecard.sid});
